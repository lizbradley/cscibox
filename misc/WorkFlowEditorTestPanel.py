...
class WorkflowPanel(wx.Panel):
    ...
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, id=-1, size=(1024, 768))
    ...
    def OnMotion(self, event):
        if event.Dragging() and event.LeftIsDown() and self.selected:
            dc = wx.BufferedDC(wx.ClientDC(self.GetParent()), self.buffer)
...
class WorkflowFrame(wx.Frame):
    def __init__(self):
        ...
        self.scroller.SetVirtualSize((1024, 768))
        self.panel    = WorkflowPanel(self.scroller)
