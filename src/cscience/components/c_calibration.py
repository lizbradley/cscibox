...
class IntCalCalibrator(cscience.components.BaseComponent):
    ...
    def prepare(self, *args, **kwargs):
        ...
        self.carbon14Ages = np.array(calibratedAgesToCarbon14Ages.values())
        self.sigmas = np.array(calibratedAgesToSigmas.values())
    ...
    def density(self, avg, error):
        sigmaSquared = error ** 2. + self.sigmas ** 2.
        exponent = -((self.carbon14Ages - avg) ** 2.) / (2.*sigmaSquared)
        alpha = 1. / np.sqrt(2.*np.pi * sigmaSquared);
    ...
    def convert_age(self, age, interval):
        ...
        unnormed_density = self.density(age.magnitude, age.uncertainty.magnitude[0].magnitude)
        ...
        annualCalibratedAges = np.array(range(int(sortedCalibratedAges[0]),int(sortedCalibratedAges[-1]+1)))
        unnormed_density = np.interp(annualCalibratedAges,sortedCalibratedAges,unnormed_density)
        sortedCalibratedAges = np.array(range(int(sortedCalibratedAges[0]),int(sortedCalibratedAges[-1]+1)))
        ...
        calibratedAgeError = self.hdr(normed_density, sortedCalibratedAges, interval)
    ...
    def hdr(self, density, years, interval):
        ...
        years_and_probability_density = np.array([years,density])
        years_and_probability_density = np.fliplr(years_and_probability_density[:,years_and_probability_density[1,:].argsort()])
        
        summation_array = np.cumsum(years_and_probability_density[1,:])
        
        index_of_win = np.searchsorted(summation_array,interval)
        good_years = years_and_probability_density[0,:index_of_win]
