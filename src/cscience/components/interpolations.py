...
class InterpolateModelLinear(cscience.components.BaseComponent):
    ...
    def run_component(self, core):
        ...
        xyvals = zip(*sorted([(sample['depth'],
                              ...
                              for sample in core]))
        self.set_value(core, 'age/depth model', datastructures.PointlistInterpolation(*xyvals))
...
class InterpolateModelSpline(cscience.components.BaseComponent):
    ...
    def run_component(self, core):
        ...
        self.set_value(core, 'age/depth model', datastructures.PointlistInterpolation(x_i, y_i))
...
class InterpolateModelRegression(cscience.components.BaseComponent):
    ...
    def run_component(self, core):
        ...
        self.set_value(core, 'age/depth model', 
                       datastructures.PointlistInterpolation([i for i in x], [y_intcpt + slope * i for i in x]))
...
class InterpolateModelCubic(cscience.components.BaseComponent):
    ...
    def run_component(self, core):
        ...
        self.set_value(core, 'age/depth model', datastructures.PointlistInterpolation(new_x, interp_func(new_x)))
